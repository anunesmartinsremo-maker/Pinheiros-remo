...
function TrainingForm({ athletes, bestTimes, onSave, onCancel, preAthlete, initial }) {
  const [f,setF]=useState(initial ? {
    ...
    spm:initial.spm||"", watts:initial.watts||"", wind:initial.wind||"Sem vento", notes:initial.notes||""
  } : {athleteId:preAthlete||"",date:todayStr(),time:"",activity:"BARCO",trainingType:"B1",series:"1",boatClass:"1x",distance:"2000",timeRowed:"",spm:"",watts:"",wind:"Sem vento",notes:""});
    ...
    <div style={{minHeight:"100vh",background:C.bg}}>
      ...
      <div style={{maxWidth:900,margin:"0 auto",padding:"16px 12px"}}>
        ...
        <div style={{display:"flex",gap:4,background:C.bg,borderRadius:8,padding:4,marginBottom:16}}>
          {[["trainings","📋 Treinos"],["steptest","🔬 Step Test"],["perfil","👤 Perfil"]].map(([k,l])=>(
        ...
        {athTab==="trainings"&&<div>
          ...
          <Card>
            <div style={{overflowX:"auto",WebkitOverflowScrolling:"touch"}}>
              <table>
                <thead><tr><th>Data</th><th>Tipo</th><th>Barco</th><th>Dist.</th><th>Tempo</th><th>VOG</th><th>% BT</th><th>Meta</th><th>Feedback</th><th></th></tr></thead>
                <tbody>
                  {mine.length===0&&<tr><td colSpan={10} style={{textAlign:"center",color:C.muted,padding:32}}>Nenhum treino registrado ainda.</td></tr>}
                  {mine.map(t=>{
                    const fb=getFB(t.trainingType,t.percBT);
                    const tgt=TARGET_PERC[String(t.trainingType||"B1").toUpperCase()]||75;
                    return <tr key={t.id}><td className="mono" style={{fontSize:11,whiteSpace:"nowrap"}}>{t.date}</td><td><span style={{background:C.surfHi,padding:"2px 8px",borderRadius:4,fontSize:11,color:C.blueLt}}>{t.trainingType}</span></td><td className="mono" style={{fontSize:11}}>{t.boatLabel}</td><td className="mono" style={{fontSize:11}}>{t.distance}m</td><td className="mono" style={{fontSize:11}}>{fmtTime(t.timeSeconds)}</td><td className="mono" style={{fontSize:11}}>{t.spm||"—"}</td><td><Badge val={t.percBT} type={t.trainingType}/></td><td className="mono" style={{fontSize:11,color:C.muted}}>{tgt}%</td><td style={{fontSize:11,color:fb?.c||C.muted}}>{fb?.msg||"—"}</td><td><button onClick={()=>{setEditingTraining(t);setShowForm(false);}} style={{background:"transparent",color:C.blueLt,border:"none",fontSize:15,cursor:"pointer",padding:"3px 6px"}}>✏️</button></td></tr>;
                  })}
                </tbody>
              </table>
            </div>
