...
import GymModule from "./GymModule.jsx";
import StepTestModule from "./StepTestModule.jsx";
import AthleteProfile360 from "./AthleteProfile360.jsx";
...
const SUB19_USER = "matias", SUB19_PASS = "sub192025"; // Treinador Sub19 (acesso restrito)
...
// Zonas de FC (mesma lógica do Step Test — método Karvonen)
const HR_ZONES = [
  { key:"B1", pctLow:0.60, pctHigh:0.75, color:"#3b82f6" },
  { key:"B2", pctLow:0.75, pctHigh:0.85, color:"#22c55e" },
  { key:"B3", pctLow:0.85, pctHigh:0.93, color:"#f59e0b" },
  { key:"B4", pctLow:0.90, pctHigh:0.95, color:"#f97316" },
  { key:"B5", pctLow:0.92, pctHigh:1.00, color:"#ef4444" },
];
function calcHRZones(maxHR, restHR) {
  if (!maxHR) return null;
  const rest = restHR || 0, reserve = maxHR - rest;
  return HR_ZONES.map(z => ({ ...z, hrLow:Math.round(rest+reserve*z.pctLow), hrHigh:Math.round(rest+reserve*z.pctHigh) }));
}
// Compara FC média do treino com a zona prescrita (B1..B5 / E1..E5 → índice 1..5)
function hrZoneFeedback(trainingType, fcAvg, zones) {
  if (!fcAvg || !zones) return null;
  const idx = parseInt(String(trainingType).replace(/[^0-9]/g,""), 10);
  if (!idx || idx < 1 || idx > 5) return null;
  const z = zones[idx-1];
  const inZone = fcAvg >= z.hrLow && fcAvg <= z.hrHigh;
  const actual = zones.find(zz => fcAvg >= zz.hrLow && fcAvg <= zz.hrHigh) ||
    (fcAvg < zones[0].hrLow ? null : zones[zones.length-1]);
  if (inZone) return { ok:true, color:"#22c55e", msg:`FC dentro da zona ${z.key} prescrita (${z.hrLow}–${z.hrHigh} bpm).`, z };
  if (fcAvg < z.hrLow) return { ok:false, color:"#3b82f6", msg:`FC abaixo da zona ${z.key} (${z.hrLow}–${z.hrHigh} bpm)${actual?` — ficou na zona ${actual.key}`:""}.`, z };
  return { ok:false, color:"#ef4444", msg:`FC acima da zona ${z.key} (${z.hrLow}–${z.hrHigh} bpm)${actual?` — ficou na zona ${actual.key}`:""}.`, z };
}
...
function dbToAthlete(r) {
  return { id:r.id, name:r.name, category:r.category, gender:r.gender||"", weight:r.weight||"", birthYear:r.birth_year,
    birthDate:r.birth_date||null, matricula:r.matricula||null, rg:r.rg||null, cpf:r.cpf||null,
    admissionDate:r.admission_date||null, shirtSize:r.shirt_size||null, shortsSize:r.shorts_size||null,
    jacketSize:r.jacket_size||null, unisuitSize:r.unisuit_size||null, c2Connected:r.c2_connected||false };
}
...
function trainingToDb(t) {
  return { id:t.id, athlete_id:t.athleteId, date:t.date, time:t.time, activity:t.activity,
    ...
    spm:t.spm, watts:t.watts,
    fc_avg:t.fcAvg===""||t.fcAvg==null?null:parseInt(t.fcAvg),
    fc_max:t.fcMax===""||t.fcMax==null?null:parseInt(t.fcMax),
    wind:t.wind, notes:t.notes, perc_bt:t.percBT,
...
function isBirthdayToday(birthDate) {
  ...
  return String(birthDate).slice(5,10) === mmdd;
}
function BirthdayOverlay({ name, onClose }) {
  const balloons = ["🎈","🎈","🎈","🎈","🎈","🎈","🎈","🎈","🎉","🎊","🎂","🎁"];
  return (
    <div onClick={onClose} style={{position:"fixed",inset:0,background:"rgba(0,0,0,.82)",zIndex:500,display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",overflow:"hidden"}}>
      <style>{`
        @keyframes bdayRise { 0%{transform:translateY(105vh) scale(.8) rotate(-6deg);opacity:0;} 12%{opacity:1;} 100%{transform:translateY(-130vh) scale(1.15) rotate(8deg);opacity:.9;} }
        @keyframes bdayPop { 0%{transform:scale(.6);opacity:0;} 60%{transform:scale(1.08);} 100%{transform:scale(1);opacity:1;} }
      `}</style>
      {balloons.map((b,i)=>(
        <div key={i} style={{position:"absolute",bottom:0,left:`${5+(i*8)%92}%`,fontSize:34+(i%4)*10,animation:`bdayRise ${6+(i%5)*1.6}s linear ${i*0.55}s infinite`,pointerEvents:"none"}}>{b}</div>
      ))}
      <div style={{textAlign:"center",animation:"bdayPop .6s ease-out",padding:24}}>
        <div style={{fontSize:64,marginBottom:8}}>🎂</div>
        <div style={{fontSize:36,fontWeight:900,color:"#fff",letterSpacing:1,lineHeight:1.15}}>Feliz Aniversário,<br/>{name}!</div>
        <div style={{fontSize:15,color:"#9db5d6",marginTop:10}}>Toda a equipe do Remo Pinheiros deseja um dia incrível! 🚣💙</div>
        <div style={{fontSize:11,color:"#4a6080",marginTop:18,letterSpacing:2}}>TOQUE PARA CONTINUAR</div>
      </div>
    </div>
  );
}
function BirthdayBanner({ athletes, excludeId }) {
  const bday = athletes.filter(a=>isBirthdayToday(a.birthDate)&&a.id!==excludeId);
  if (!bday.length) return null;
  const names = bday.map(a=>firstName(a.name)).join(", ");
  return (
    <div style={{background:"linear-gradient(90deg,rgba(26,79,160,.25),rgba(37,99,196,.12))",border:`1px solid ${C.blueLt}`,borderRadius:8,padding:"10px 14px",marginBottom:14,fontSize:13,fontWeight:700,color:C.text,display:"flex",alignItems:"center",gap:8}}>
      <span style={{fontSize:18}}>🎂</span> Hoje é aniversário de <span style={{color:C.blueLt}}>{names}</span>! Deixe seus parabéns! 🎉
    </div>
  );
}
...
function ProximityRanking({ title, trainings, athletes, period }) {
  ...
  const ranked = Object.entries(byAthlete).map(([id,d]) => {
    ...
    if (a.reached&&b.reached) return b.score-a.score;
...
function Login({ athletes, onLogin }) {
  ...
  function go() {
    ...
    if (tab==="admin") {
      ...
      else if (user===SUB19_USER&&pass===SUB19_PASS) onLogin({role:"sub19_coach"});
...
function TrainingForm({ athletes, bestTimes, onSave, onCancel, preAthlete, initial }) {
  const [f,setF]=useState(initial ? {
    athleteId:initial.athleteId, date:initial.date, time:initial.time||"",
    activity:initial.activity||"BARCO", trainingType:initial.trainingType||"B1",
    series:initial.series||"1", boatClass:initial.boatClass||"1x",
    distance:initial.distance||"2000", timeRowed:initial.timeRowed||"",
    spm:initial.spm||"", watts:initial.watts||"", fcAvg:initial.fcAvg||"", fcMax:initial.fcMax||"", wind:initial.wind||"Sem vento", notes:initial.notes||""
  } : {athleteId:preAthlete||"",date:todayStr(),time:"",activity:"BARCO",trainingType:"B1",series:"1",boatClass:"1x",distance:"2000",timeRowed:"",spm:"",watts:"",fcAvg:"",fcMax:"",wind:"Sem vento",notes:""});
  const [saving,setSaving]=useState(false);
  const [zoneInfo,setZoneInfo]=useState(null);
  const set=(k,v)=>setF(p=>({...p,[k]:v}));

  // Busca o step test mais recente do atleta para comparar a FC com as zonas
  useEffect(()=>{
    let on=true;
    if (!f.athleteId) { setZoneInfo(null); return; }
    (async()=>{
      const {data} = await supabase.from("step_tests")
        .select("test_date,max_hr,rest_hr")
        .eq("athlete_id",f.athleteId)
        .order("test_date",{ascending:false}).limit(1);
      if (!on) return;
      const t = data&&data[0];
      setZoneInfo(t&&t.max_hr ? { testDate:t.test_date, zones:calcHRZones(t.max_hr,t.rest_hr) } : null);
    })();
    return ()=>{on=false;};
  },[f.athleteId]);
  ...
  const fcAvgN=f.fcAvg===""?null:parseInt(f.fcAvg);
  const fcMaxN=f.fcMax===""?null:parseInt(f.fcMax);
  const hrFb=hrZoneFeedback(f.trainingType, fcAvgN, zoneInfo?.zones);
  async function save() {
    if (!f.athleteId||!f.timeRowed||!f.distance){alert("Preencha atleta, distância e tempo.");return;}
    if (fcAvgN!=null&&(fcAvgN<30||fcAvgN>250)){alert("FC Média fora do intervalo válido (30–250 bpm).");return;}
    if (fcMaxN!=null&&(fcMaxN<30||fcMaxN>250)){alert("FC Máxima fora do intervalo válido (30–250 bpm).");return;}
    if (fcAvgN!=null&&fcMaxN!=null&&fcMaxN<fcAvgN){alert("FC Máxima não pode ser menor que a FC Média.");return;}
    setSaving(true);
    const rec={id:initial?.id||uid(),...f,timeSeconds:tS,speedMs:spd,percBT:perc,boatLabel,btLabel,btSpeed:bt?.speedMs||null,createdAt:initial?.createdAt||new Date().toISOString()};
    await onSave(rec);
    setSaving(false);
  }
  return (
    <div>
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(155px,1fr))",gap:12}}>
        ...
        <Field label="FC Média (bpm)"><input type="number" value={f.fcAvg} onChange={e=>set("fcAvg",e.target.value)} placeholder="150"/></Field>
        <Field label="FC Máxima (bpm)"><input type="number" value={f.fcMax} onChange={e=>set("fcMax",e.target.value)} placeholder="172"/></Field>
      ...
      {fcAvgN!=null&&hrFb&&(
        <div style={{background:C.surfHi,borderRadius:8,padding:"10px 14px",marginTop:14,border:`1px solid ${hrFb.color}`,display:"flex",alignItems:"center",gap:10,flexWrap:"wrap"}}>
          <span style={{fontSize:16}}>❤️</span>
          <div style={{flex:1,minWidth:200}}>
            <div style={{fontSize:13,color:hrFb.color,fontWeight:700}}>{hrFb.msg}</div>
            <div style={{fontSize:11,color:C.muted,marginTop:2}}>Zonas do step test de {zoneInfo?.testDate?new Date(zoneInfo.testDate+"T12:00:00").toLocaleDateString("pt-BR"):"—"}</div>
          </div>
          <span className="mono" style={{fontSize:18,fontWeight:800,color:hrFb.color}}>{fcAvgN} bpm</span>
        </div>
      )}
      {fcAvgN!=null&&!zoneInfo&&(
        <div style={{background:C.surfHi,borderRadius:8,padding:"10px 14px",marginTop:14,border:`1px solid ${C.border}`,fontSize:12,color:C.muted}}>
          ❤️ FC registrada, mas o atleta ainda não tem step test com FC máxima — sem zonas para comparar.
        </div>
      )}
      {perc!=null&&(
...
// ─── ATHLETE VIEW ─────────────────────────────────────────────────────────
function AthleteView({ session, athletes, trainings, bestTimes, onLogout, onAdd, onUpdate }) {
  ...
  const isMyBirthday=isBirthdayToday(ath?.birthDate);
  const mine=[...trainings.filter(t=>t.athleteId===session.athleteId)].sort((a,b)=>b.createdAt.localeCompare(a.createdAt));

  async function syncC2() {
    setSyncingC2(true);
    try {
      const r = await fetch(`/api/c2/sync?athlete=${session.athleteId}`);
      const j = await r.json();
      if (j.error) alert("Erro ao sincronizar: "+j.error);
      else alert(j.imported>0?`✅ ${j.imported} treino(s) importado(s) do Concept2!`:"Nenhum treino novo no Concept2.");
    } catch(e) { alert("Erro ao sincronizar: "+e.message); }
    setSyncingC2(false);
  }

  // Cobrança de musculação: banner se passou de 5 dias sem registro
  const [gymNudge,setGymNudge]=useState(null);
  useEffect(()=>{
    (async()=>{
      try {
        const dism = localStorage.getItem("ecp_gym_nudge_until");
        if (dism && Date.now() < parseInt(dism)) return;
      } catch {}
      const {data} = await supabase.from("gym_records").select("date")
        .eq("athlete_id",session.athleteId).order("date",{ascending:false}).limit(1);
      const last = data&&data[0]?.date;
      const days = last ? Math.floor((Date.now()-new Date(last+"T12:00:00").getTime())/86400000) : null;
      if (!last || days>=5) setGymNudge({days,last});
    })();
  },[session.athleteId]);
  function dismissGymNudge() {
    try { localStorage.setItem("ecp_gym_nudge_until", String(Date.now()+4*86400000)); } catch {}
    setGymNudge(null);
  }

  async function handleUpdateTraining(rec) {
    const {error} = await supabase.from("trainings").update(trainingToDb(rec)).eq("id",rec.id);
  ...
  return (
    <div style={{minHeight:"100vh",background:C.bg}}>
      <style>{GS}</style>
      {isMyBirthday&&showBday&&<BirthdayOverlay name={firstName(ath.name)} onClose={()=>setShowBday(false)}/>}
      ...
      <div style={{maxWidth:900,margin:"0 auto",padding:"16px 12px"}}>
        <BirthdayBanner athletes={athletes} excludeId={session.athleteId}/>
        {gymNudge&&athTab!=="gym"&&(
          <div style={{background:"rgba(245,158,11,.1)",border:`1px solid ${C.warn}`,borderRadius:8,padding:"10px 14px",marginBottom:14,display:"flex",alignItems:"center",gap:10,flexWrap:"wrap"}}>
            <span style={{fontSize:18}}>🏋️</span>
            <div style={{flex:1,minWidth:180,fontSize:13,color:C.text}}>
              {gymNudge.last
                ? <>Sua última carga de musculação foi registrada há <b style={{color:C.warn}}>{gymNudge.days} dias</b>. Não deixe acumular — registre logo após a sessão!</>
                : <>Você ainda não registrou nenhuma carga de musculação. Comece hoje!</>}
            </div>
            <Btn sz="sm" onClick={()=>{setAthTab("gym");setGymNudge(null);}}>Registrar agora</Btn>
            <button onClick={dismissGymNudge} style={{background:"transparent",border:"none",color:C.muted,fontSize:11,cursor:"pointer",textDecoration:"underline"}}>Estamos em pausa</button>
          </div>
        )}
        ...
        <div style={{display:"flex",gap:4,background:C.bg,borderRadius:8,padding:4,marginBottom:16}}>
          {[["trainings","📋 Treinos"],["gym","🏋️ Musculação"],["wellness","💚 Bem-Estar"],["steptest","🔬 Step Test"],["perfil","👤 Perfil"]].map(([k,l])=>(
        ...
        {athTab==="perfil"&&<AthleteProfile360 athletes={athletes} session={session}/>}
        {athTab==="wellness"&&<WellnessForm session={session} athletes={athletes}/>}
        {athTab==="gym"&&<GymModule session={session} athletes={athletes}/>}
        {athTab==="steptest"&&<StepTestModule session={session} athletes={athletes}/>}
        {athTab==="trainings"&&<div>
          <Card s={{marginBottom:14,display:"flex",alignItems:"center",gap:12,flexWrap:"wrap",borderLeft:`3px solid ${ath?.c2Connected?C.good:C.blueLt}`}}>
            <span style={{fontSize:22}}>⌚</span>
            <div style={{flex:1,minWidth:180}}>
              <div style={{fontWeight:800,fontSize:14}}>Concept2 Logbook</div>
              <div style={{fontSize:11,color:C.muted}}>
                {ath?.c2Connected
                  ? "Conta conectada — seus treinos de ergômetro entram automaticamente todo dia. Você também pode continuar registrando manualmente."
                  : "Conecte sua conta para importar treinos de ergômetro automaticamente. O registro manual continua disponível."}
              </div>
            </div>
            {ath?.c2Connected
              ? <Btn sz="sm" variant="outline" onClick={syncC2} disabled={syncingC2}>{syncingC2?"Sincronizando...":"↻ Sincronizar agora"}</Btn>
              : <Btn sz="sm" onClick={()=>{window.location.href=`/api/c2/auth?athlete=${session.athleteId}`;}}>🔗 Conectar</Btn>}
          </Card>
          ...
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12,marginTop:16}}>
            ...
            <Btn sz="sm" onClick={()=>{setShowForm(v=>!v);setEditingTraining(null);}}>{showForm?"✕ Fechar":"+ Registrar"}</Btn>
          </div>
          ...
          {editingTraining&&<Card s={{marginBottom:14,border:`1px solid ${C.blueLt}`}}><Lbl>Editar Treino</Lbl><div style={{marginBottom:10}}/><TrainingForm athletes={athletes} bestTimes={bestTimes} preAthlete={session.athleteId} initial={editingTraining} onSave={handleUpdateTraining} onCancel={()=>setEditingTraining(null)}/></Card>}
          <Card>
            <div style={{overflowX:"auto",WebkitOverflowScrolling:"touch"}}>
              <table>
                <thead><tr><th>Data</th><th>Tipo</th><th>Barco</th><th>Dist.</th><th>Tempo</th><th>VOG</th><th>FC</th><th>% BT</th><th>Meta</th><th>Feedback</th><th></th></tr></thead>
                <tbody>
                  {mine.length===0&&<tr><td colSpan={11} style={{textAlign:"center",color:C.muted,padding:32}}>Nenhum treino registrado ainda.</td></tr>}
                  {mine.map(t=>{
                    const fb=getFB(t.trainingType,t.percBT);
                    const tgt=TARGET_PERC[String(t.trainingType||"B1").toUpperCase()]||75;
                    return <tr key={t.id}><td className="mono" style={{fontSize:11,whiteSpace:"nowrap"}}>{t.date}</td><td><span style={{background:C.surfHi,padding:"2px 8px",borderRadius:4,fontSize:11,color:C.blueLt}}>{t.trainingType}</span></td><td className="mono" style={{fontSize:11}}>{t.boatLabel}</td><td className="mono" style={{fontSize:11}}>{t.distance}m</td><td className="mono" style={{fontSize:11}}>{fmtTime(t.timeSeconds)}</td><td className="mono" style={{fontSize:11}}>{t.spm||"—"}</td><td className="mono" style={{fontSize:11,whiteSpace:"nowrap"}}>{t.fcAvg?<span style={{color:"#ef4444"}}>{t.fcAvg}{t.fcMax?`/${t.fcMax}`:""}</span>:"—"}</td><td><Badge val={t.percBT} type={t.trainingType}/></td><td className="mono" style={{fontSize:11,color:C.muted}}>{tgt}%</td><td style={{fontSize:11,color:fb?.c||C.muted}}>{fb?.msg||"—"}</td><td><button onClick={()=>{setEditingTraining(t);setShowForm(false);}} style={{background:"transparent",color:C.blueLt,border:"none",fontSize:15,cursor:"pointer",padding:"3px 6px"}}>✏️</button></td></tr>;
                  })}
                </tbody>
              </table>
            </div>
...
// ─── DASHBOARD ────────────────────────────────────────────────────────────
function Dashboard({ athletes, trainings, onRefresh }) {
  const [rankTab,setRankTab]=useState("day");
  ...
  return (
    <div>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
        ...
        <Btn variant="ghost" sz="sm" onClick={onRefresh}>↻ Atualizar</Btn>
      </div>
...
function AllTrainings({ athletes, trainings, bestTimes, onAdd, onDelete }) {
  ...
  async function syncAllC2() {
    setSyncingC2(true);
    try {
      const r = await fetch("/api/c2/sync");
      const j = await r.json();
      if (j.error) alert("Erro: "+j.error);
      else alert(`Sincronização Concept2: ${j.imported} treino(s) novo(s) importado(s).${j.problems?"\n\nAvisos:\n"+j.problems.join("\n"):""}`);
    } catch(e) { alert("Erro ao sincronizar: "+e.message); }
    setSyncingC2(false);
  }
  const filtered=[...trainings].filter(t=>{
    ...
    <div>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
        ...
        <div style={{display:"flex",gap:8}}>
          <Btn variant="outline" sz="sm" onClick={syncAllC2} disabled={syncingC2}>{syncingC2?"Sincronizando...":"⌚ Sincronizar Concept2"}</Btn>
      ...
      <Card>
        <div style={{overflowX:"auto",WebkitOverflowScrolling:"touch"}}>
          <table>
            <thead><tr><th>Data</th><th>Atleta</th><th>Tipo</th><th>Barco</th><th>Dist.</th><th>Tempo</th><th>VOG</th><th>FC</th><th>m/s</th><th>% BT</th><th>Meta</th><th>Vento</th><th></th></tr></thead>
            <tbody>
              {filtered.length===0&&<tr><td colSpan={13} style={{textAlign:"center",color:C.muted,padding:28}}>Nenhum resultado.</td></tr>}
              {filtered.map(t=>{
                const a=athletes.find(x=>x.id===t.athleteId);
                const tgt=TARGET_PERC[String(t.trainingType||"B1").toUpperCase()]||75;
                return <tr key={t.id}><td className="mono" style={{fontSize:11,whiteSpace:"nowrap"}}>{t.date}</td><td style={{fontSize:12,fontWeight:600,whiteSpace:"nowrap"}}>{a?firstName(a.name):"?"}</td><td><span style={{background:C.surfHi,padding:"2px 7px",borderRadius:4,fontSize:11,color:C.blueLt}}>{t.trainingType}</span></td><td className="mono" style={{fontSize:11}}>{t.boatLabel}</td><td className="mono" style={{fontSize:11}}>{t.distance}m</td><td className="mono" style={{fontSize:11}}>{fmtTime(t.timeSeconds)}</td><td className="mono" style={{fontSize:11}}>{t.spm||"—"}</td><td className="mono" style={{fontSize:11,whiteSpace:"nowrap"}}>{t.fcAvg?<span style={{color:"#ef4444"}}>{t.fcAvg}{t.fcMax?`/${t.fcMax}`:""}</span>:"—"}</td><td className="mono" style={{fontSize:11}}>{t.speedMs?.toFixed(3)||"—"}</td><td><Badge val={t.percBT} type={t.trainingType}/></td><td><span className="mono" style={{fontSize:11,color:C.muted}}>{tgt}%</span>{(t.percBT||0)>=tgt&&<span style={{marginLeft:4,color:C.good,fontSize:10}}>✓</span>}</td><td style={{fontSize:11,color:C.muted,whiteSpace:"nowrap"}}>{t.wind}</td><td><button onClick={()=>{if(confirm("Excluir?"))onDelete(t.id);}} style={{background:"transparent",color:C.bad,border:"none",fontSize:15,cursor:"pointer",padding:"3px 6px"}}>🗑</button></td></tr>;
              })}
            </tbody>
          </table>
        </div>
...
// ─── ATHLETES ADMIN ───────────────────────────────────────────────────────
function AthletesAdmin({ athletes, onAdd, onDelete, onUpdate }) {
  ...
  async function saveEdit(){
    setSaving(true);
    await onUpdate({...ef,birthYear:parseInt(ef.birthYear)||null});
  ...
  return (
    <div>
      ...
      {editing&&<Card s={{marginBottom:14,border:`1px solid ${C.blueLt}`}}>
        <div style={{fontSize:12,fontWeight:700,color:C.blueLt,letterSpacing:2,marginBottom:12}}>EDITANDO ATLETA</div>
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(155px,1fr))",gap:12,marginBottom:12}}>
          <Field label="Nome completo" span={2}><input value={ef.name} onChange={e=>setE("name",e.target.value)}/></Field>
          <Field label="Categoria"><input value={ef.category} onChange={e=>setE("category",e.target.value)}/></Field>
          <Field label="Gênero"><select value={ef.gender} onChange={e=>setE("gender",e.target.value)}><option>Masc</option><option>Fem</option><option>Masc Para</option><option>PR1 Fem Para</option></select></Field>
          <Field label="Peso Leve"><select value={ef.weight} onChange={e=>setE("weight",e.target.value)}><option value="">Não</option><option value="Peso Leve">Sim</option></select></Field>
          <Field label="Ano Nasc."><input type="number" value={ef.birthYear||""} onChange={e=>setE("birthYear",e.target.value)}/></Field>
        </div>
        <div style={{display:"flex",gap:8,justifyContent:"flex-end"}}>
          <Btn variant="ghost" sz="sm" onClick={()=>setEditing(null)} disabled={saving}>Cancelar</Btn>
          <Btn sz="sm" onClick={saveEdit} disabled={saving}>{saving?"Salvando...":"✓ Salvar Edição"}</Btn>
        </div>
      </Card>}
      <Card>
        <div style={{overflowX:"auto"}}>
              ...
              {athletes.map(a=>(
                <tr key={a.id}>
                  ...
                  <td style={{display:"flex",gap:6,whiteSpace:"nowrap"}}>
                    <button onClick={()=>{setEditing(a.id);setEf({...a});setShowAdd(false);}} style={{background:"transparent",color:C.blueLt,border:"none",fontSize:15,cursor:"pointer"}}>✏️</button>
                    <button onClick={()=>{if(confirm("Remover atleta?"))onDelete(a.id);}} style={{background:"transparent",color:C.bad,border:"none",fontSize:15,cursor:"pointer"}}>🗑</button>
                  </td>
                </tr>
              ))}
            </tbody>
...
// ─── ADMIN LAYOUT ─────────────────────────────────────────────────────────
function AdminView({ session, athletes, trainings, bestTimes, onLogout, onAddAthlete, onDeleteAthlete, onUpdateAthlete, onAddTraining, onDeleteTraining, onUpdateBT, onAddBT, onDeleteBT, onRefresh }) {
  const [tab,setTab]=useState("dashboard");
  // ── RBAC: treinador Sub19 (Matias) vê apenas atletas Sub19 e abas permitidas ──
  const isSub19Coach = session?.role==="sub19_coach";
  if (isSub19Coach) {
    athletes  = athletes.filter(a=>(a.category||"").includes("Sub19"));
    trainings = trainings.filter(t=>athletes.some(a=>a.id===t.athleteId));
  }
  const ALL_TABS=[{id:"dashboard",icon:"📊",label:"Dashboard"},{id:"perfil",icon:"👤",label:"Perfil 360"},{id:"trainings",icon:"📋",label:"Treinos"},{id:"gym",icon:"🏋️",label:"Musculação"},{id:"athletes",icon:"🚣",label:"Atletas"},{id:"besttimes",icon:"🏆",label:"Best Times"},{id:"wellness",icon:"💚",label:"Bem-Estar"},{id:"steptest",icon:"🔬",label:"Step Test"}];
  const TABS = isSub19Coach ? ALL_TABS.filter(t=>!["athletes","besttimes"].includes(t.id)) : ALL_TABS;
  return (
    <div style={{minHeight:"100vh",display:"flex",flexDirection:"column",background:C.bg}}>
      ...
      <div style={{flex:1,maxWidth:1200,margin:"0 auto",width:"100%",padding:"20px 12px"}}>
        <BirthdayBanner athletes={athletes}/>
        {tab==="dashboard"&&<Dashboard athletes={athletes} trainings={trainings} onRefresh={onRefresh}/>}
        {tab==="perfil"&&<AthleteProfile360 athletes={athletes} session={session}/>}
        ...
        {tab==="gym"&&<GymModule session={session} athletes={athletes}/>}
        {tab==="athletes"&&<AthletesAdmin athletes={athletes} onAdd={onAddAthlete} onDelete={onDeleteAthlete} onUpdate={onUpdateAthlete}/>}
...
export default function App() {
  const [session,  setSession]  = useState(()=>{
    try { const s=localStorage.getItem("ecp_session"); return s?JSON.parse(s):null; } catch { return null; }
  });
  ...
  function handleLogin(s) { setSession(s); try { localStorage.setItem("ecp_session",JSON.stringify(s)); } catch {} }
  function handleLogout() { setSession(null); try { localStorage.removeItem("ecp_session"); } catch {} }

  // Mensagem de retorno da conexão Concept2
  useEffect(()=>{
    const p = new URLSearchParams(window.location.search);
    if (p.get("c2")==="ok") { alert("✅ Conta Concept2 conectada com sucesso! Seus treinos de ergômetro serão importados automaticamente."); window.history.replaceState({},"",window.location.pathname); }
    else if (p.get("c2")==="erro") { alert("❌ Não foi possível conectar a conta Concept2. Tente novamente."); window.history.replaceState({},"",window.location.pathname); }
  },[]);
  ...
  const updateTraining = useCallback(async(rec)=>{
    const {error} = await supabase.from("trainings").update(trainingToDb(rec)).eq("id",rec.id);
    if (error) { alert("Erro ao atualizar treino: "+error.message); return; }
    setTrainings(p=>p.map(t=>t.id===rec.id?rec:t));
  },[]);
  ...
  const updateAthlete = useCallback(async(a)=>{
    const {error} = await supabase.from("athletes").update(athleteToDb(a)).eq("id",a.id);
    if (error) { alert("Erro: "+error.message); return; }
    setAthletes(p=>p.map(x=>x.id===a.id?a:x).sort((a,b)=>a.name.localeCompare(b.name)));
  },[]);
  ...
  if (!session) return <Login athletes={athletes} onLogin={handleLogin}/>;
  if (session.role==="athlete") return (
    <AthleteView session={session} athletes={athletes} trainings={trainings} bestTimes={bestTimes}
      onLogout={handleLogout} onAdd={addTraining} onUpdate={updateTraining}/>
  );
  return (
    <AdminView session={session}
      ...
      onAddAthlete={addAthlete}     onDeleteAthlete={deleteAthlete}   onUpdateAthlete={updateAthlete}
