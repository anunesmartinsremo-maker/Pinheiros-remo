...
// ─── CONFIG (Google Sheets — usado APENAS para importar o histórico antigo) ─
...
export default function WellnessDashboard() {
  ...
  // Converte linha do Supabase para o formato que o painel já entende
  function dbToEntry(r) {
    return {
      Timestamp: `${r.date} ${r.time || "00:00:00"}`,
      ID_Atleta: r.athlete_id,
      Score_Wellness: r.score != null ? String(r.score) : "",
      HRV_VFC: r.hrv != null ? String(r.hrv) : "",
      FC_Repouso: r.rest_hr != null ? String(r.rest_hr) : "",
      Qualidade_Sono: r.sleep_quality != null ? String(r.sleep_quality) : "",
      Fadiga: r.fatigue != null ? String(r.fatigue) : "",
      Dor_Muscular: r.soreness != null ? String(r.soreness) : "",
      Motivacao: r.motivation != null ? String(r.motivation) : "",
      Estado_Geral: r.comments || "",
    };
  }
  ...
  async function load() {
    try {
      ...
      const [{ data: ath, error: e1 }, { data: well, error: e2 }] = await Promise.all([
        supabase.from("athletes").select("*").order("name"),
        supabase.from("wellness_responses").select("*").order("date", { ascending: true }),
      ]);
      if (e1) throw e1;
      if (e2) throw e2;
      setAthletes((ath || []).map(a => ({ ID: a.id, Nome: a.name, Categoria: a.category })));
      setWellness((well || []).map(dbToEntry));
  ...
  // ─── IMPORTAÇÃO DO HISTÓRICO (Google Sheets → Supabase, uma única vez) ───
  const [importing, setImporting] = useState(false);
  function norm(s) {
    return String(s||"").toUpperCase().normalize("NFD").replace(/[\u0300-\u036f]/g,"").trim();
  }
  async function importHistory() {
    if (!confirm("Importar todo o histórico do Google Sheets para o banco? (seguro repetir — não duplica)")) return;
    setImporting(true);
    try {
      const [sheetAth, sheetWell] = await Promise.all([
        fetchSheet("ATLETAS"),
        fetchSheet("RESPOSTAS_WELLNESS"),
      ]);
      const { data: dbAth } = await supabase.from("athletes").select("id,name");
      // mapa: ID da planilha → id do Supabase (casado por nome)
      const idMap = {};
      sheetAth.forEach(sa => {
        const n = norm(sa.Nome);
        const match = (dbAth||[]).find(da => {
          const dn = norm(da.name);
          return dn === n || dn.includes(n) || n.includes(dn);
        });
        if (match) idMap[(sa.ID||"").trim()] = match.id;
      });
      const num = v => { const x = parseFloat(String(v||"").replace(",", ".")); return isNaN(x)?null:x; };
      const int = v => { const x = parseInt(String(v||"").replace(",", ".")); return isNaN(x)?null:x; };
      const rows = [];
      let skipped = 0;
      sheetWell.forEach(w => {
        const aid = idMap[(w.ID_Atleta||"").trim()];
        if (!aid || !w.Timestamp) { skipped++; return; }
        const [dp, tp] = w.Timestamp.split(" ");
        let iso = dp;
        if (dp.includes("/")) {
          const [d,m,y] = dp.split("/");
        ...
        rows.push({
          ...
          date: iso,
          time: tp || "00:00:00",
          sleep_quality: int(w.Qualidade_Sono),
          fatigue: int(w.Fadiga),
          soreness: int(w.Dor_Muscular),
          motivation: int(w.Motivacao),
          score: num(w.Score_Wellness),
          hrv: int(w.HRV_VFC),
          rest_hr: int(w.FC_Repouso),
          comments: w.Estado_Geral || null,
        });
      });
      // upsert em lotes de 200 (id determinístico ⇒ rodar 2x não duplica)
      for (let i = 0; i < rows.length; i += 200) {
        const { error } = await supabase.from("wellness_responses").upsert(rows.slice(i, i+200));
        if (error) throw error;
      }
      alert(`Importação concluída: ${rows.length} registros importados${skipped?`, ${skipped} ignorados (atleta não encontrado)`:""}.`);
      await load();
    } catch(e) {
  ...
  if (error) return (
    <div style={{padding:32,textAlign:"center",maxWidth:440,margin:"0 auto"}}>
      ...
      <div style={{color:C.muted,fontSize:12,marginBottom:16}}>
        Não foi possível ler os dados de bem-estar no banco.<br/>
        <span style={{color:C.blueLt,fontSize:11}}>Verifique a conexão e se a tabela wellness_responses foi criada no Supabase.</span>
      </div>
    ...
    <div style={{minHeight:"100%",background:C.bg}}>
      ...
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14,flexWrap:"wrap",gap:8}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:10}}>
          ...
          {wellness.length===0&&(
            <button onClick={importHistory} disabled={importing} style={{background:"#16a34a",border:"none",color:"#fff",borderRadius:6,padding:"5px 12px",cursor:"pointer",fontSize:12,fontFamily:"inherit",fontWeight:700,opacity:importing?.6:1}}>
              {importing?"Importando...":"⬇ Importar histórico do Sheets"}
            </button>
          )}
