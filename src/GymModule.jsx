...
function GymEvolution({ athlete, records, exercises }) {
  ...
  // Alvos por grupo (melhor 1RM projetado de exercício-chave nos últimos 90 dias)
  ...
  // Série temporal do exercício selecionado (melhor e1rm por data)
  const points = useMemo(()=>{
    const byDate = {};
    mine.filter(r=>r.exercise===selEx&&r.e1rm).forEach(r=>{
  ...
  const allExNames = [...new Set(mine.map(r=>r.exercise))].sort();
    ...
    <div>
      ...
      <Card s={{marginBottom:16}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:8,marginBottom:10}}>
          <div style={{fontSize:14,fontWeight:800}}>📈 Evolução do 1RM Projetado</div>
          <select value={selEx} onChange={e=>setSelEx(e.target.value)}
            ...
            {allExNames.filter(n=>!keyExs.some(e=>e.name===n)).map(n=><option key={n} value={n}>{n}</option>)}
          </select>
        </div>
        <EvoChart points={points}/>
        <div style={{fontSize:10,color:C.muted,marginTop:6}}>1RM projetado pela tabela de repetições (8 reps = 80%, 10 reps = 75%) — permite comparar períodos com séries diferentes.</div>
      </Card>
            ...
            <tbody>
              ...
              {prs.map(r=>{
                ...
                return <tr key={r.exercise}>
                  ...
                  <td className="mono" style={{padding:"8px 10px",borderBottom:`1px solid ${C.border}`,fontSize:12,color:C.muted}}>{r.load_kg} kg × {r.reps||"?"}</td>
