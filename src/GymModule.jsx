...
function EvoChart({ points, color=C.blueLt }) {
  // points: [{date, val}] ordenado por data
...
function GymEvolution({ athlete, records, exercises }) {
  ...
  const allExNames = useMemo(()=>[...new Set(mine.map(r=>r.exercise))].sort(),[mine]);
  ...
  // Série temporal (múltiplos exercícios ou um)
  const points = useMemo(()=>{
    const byDate = {};
    selEx.forEach(exName=>{
      mine.filter(r=>r.exercise===exName&&r.e1rm).forEach(r=>{
        if (!byDate[r.date]) byDate[r.date] = {};
        if (!byDate[r.date][exName] || r.e1rm > byDate[r.date][exName]) {
          byDate[r.date][exName] = r.e1rm;
        }
      });
    });
  ...
  function toggleEx(ex) {
    setSelEx(p => p.includes(ex) ? p.filter(x=>x!==ex) : [...p,ex]);
  }
    ...
    <div>
              ...
              <>
                ...
                {pct!=null&&(
                  <div style={{marginTop:6}}>
                    ...
                    <div style={{fontSize:9,color:C.muted,marginTop:2,lineHeight:1.4}}>Cálculo: (1RM ÷ peso corporal) ÷ alvo da categoria × 100. Exemplo: 296% = você consegue fazer 2,96× seu peso, e o alvo é 1,0× → 2,96÷1,0×100.</div>
                  </div>
                )}
              </>
            )}
      ...
      <Card s={{marginBottom:16}}>
        <div style={{marginBottom:10}}>
          <div style={{fontSize:14,fontWeight:800,marginBottom:8}}>📈 Evolução do 1RM Projetado (últimos 12 meses)</div>
          <div style={{fontSize:11,color:C.muted,marginBottom:10}}>Selecione quais exercícios mostrar (clique para ativar/desativar):</div>
          <div style={{display:"flex",flexWrap:"wrap",gap:6}}>
            {allExNames.map(n=>{
              const isKey = keyExs.some(e=>e.name===n);
              const isSelected = selEx.includes(n);
              return (
                <button key={n} onClick={()=>toggleEx(n)}
                  style={{padding:"6px 12px",background:isSelected?C.blue:C.surfHi,color:isSelected?"#fff":C.muted,border:`1px solid ${isSelected?C.blue:C.border}`,borderRadius:6,fontSize:11,fontWeight:700,cursor:"pointer",fontFamily:"inherit"}}>
                  {isKey&&"⭐ "}{n}
                </button>
              );
            })}
        ...
        {selEx.length>0 && points.length>0 ? <EvoChartMulti points={points} exercises={selEx}/> : <div style={{color:C.muted,fontSize:12,textAlign:"center",padding:"24px 0"}}>Selecione pelo menos um exercício.</div>}
