...
// ─── TEST REPORT VIEW ─────────────────────────────────────────────────────
function TestReport({ test, steps, prevTest, prevSteps, onBack, isCoach, onEdit }) {
  ...
  return (
    <div>
      ...
      <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:16,flexWrap:"wrap"}}>
        ...
        {isCoach && <Btn variant="outline" sz="sm" onClick={onEdit}>✏️ Editar</Btn>}
      </div>
...
// ─── FORM NOVO / EDITAR STEP TEST ─────────────────────────────────────────
function NewTestForm({ athleteId, athletes, onSave, onCancel, editTest, editSteps }) {
  const isEdit = !!editTest;
  const v = (x) => x == null ? "" : String(x);
  const initSteps = isEdit
    ? [...editSteps].filter(s=>s.step_number!=="Max").sort((a,b)=>parseInt(a.step_number)-parseInt(b.step_number))
        .map(s=>({ step_number:String(s.step_number), power_w:v(s.power_w), hr_bpm:v(s.hr_bpm), distance_m:v(s.distance_m), stroke_rate:v(s.stroke_rate), lactate:v(s.lactate), rpe:v(s.rpe) }))
    : null;
  const initMax = isEdit
    ? (()=>{ const m = editSteps.find(s=>s.step_number==="Max") || {};
        return { power_w:v(m.power_w), hr_bpm:v(m.hr_bpm), distance_m:v(m.distance_m), stroke_rate:v(m.stroke_rate), lactate:v(m.lactate), rpe:v(m.rpe) }; })()
    : null;
  const [info, setInfo] = useState({
    athlete_id: isEdit ? editTest.athlete_id : (athleteId || ""),
    test_date: isEdit ? editTest.test_date : new Date().toISOString().slice(0,10),
    protocol: isEdit ? (editTest.protocol || "StepTestSlide") : "StepTestSlide",
    body_mass: isEdit ? v(editTest.body_mass) : "",
    rest_hr: isEdit ? v(editTest.rest_hr) : "",
    temp: isEdit ? v(editTest.temp) : "",
    notes: isEdit ? (editTest.notes || "") : "",
  });
  const [numSteps, setNumSteps] = useState(isEdit ? Math.max(initSteps.length, 3) : 6);
  const [steps, setSteps] = useState(
    isEdit
      ? Array.from({length:Math.max(initSteps.length,3)}, (_,i) => initSteps[i] || { step_number:String(i+1), power_w:"", hr_bpm:"", distance_m:"", stroke_rate:"", lactate:"", rpe:"" })
      : Array.from({length:6}, (_,i) => ({ step_number: String(i+1), power_w:"", hr_bpm:"", distance_m:"", stroke_rate:"", lactate:"", rpe:"" }))
  );
  const [maxStep, setMaxStep] = useState(initMax || { power_w:"", hr_bpm:"", distance_m:"", stroke_rate:"", lactate:"", rpe:"" });
  ...
  async function save() {
    ...
    setSaving(true);
    try {
      const testId = isEdit ? editTest.id : uid();
      ...
      const testRow = {
        ...
        max_rel_power_corrg: isEdit ? (editTest.max_rel_power_corrg ?? null) : null,
      ...
      if (isEdit) {
        const { error: e1 } = await supabase.from("step_tests").update(testRow).eq("id", testId);
        if (e1) throw e1;
        const { error: eDel } = await supabase.from("step_test_steps").delete().eq("test_id", testId);
        if (eDel) throw eDel;
      } else {
        const { error: e1 } = await supabase.from("step_tests").insert({ id: testId, ...testRow, created_by: "user" });
        if (e1) throw e1;
      }
  ...
  return (
    <div>
      ...
      <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:16}}>
        ...
        <div style={{fontSize:20,fontWeight:800}}>{isEdit ? "✏️ Editar Step Test" : "Novo Step Test"}</div>
      </div>
...
export default function StepTestModule({ session, athletes }) {
  ...
  // ── VIEW: EDITAR TESTE (somente treinador) ──
  if (view === "edit" && selTest && isCoach) {
    const testSteps = steps.filter(s => s.test_id === selTest.id);
    return (
      <NewTestForm
        athleteId={selTest.athlete_id}
        athletes={athletes}
        editTest={selTest}
        editSteps={testSteps}
        onSave={async () => { await load(); setSelTest(null); setView("athlete"); }}
